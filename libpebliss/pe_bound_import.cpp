...
namespace pe_bliss
{
	...
	//BOUND IMPORT
	//Default constructor
	bound_import_ref::bound_import_ref() noexcept
		:timestamp_(0)
	{}
	...
	const std::string& bound_import_ref::get_module_name() const noexcept
	{
		return module_name_;
	}

	//Returns bound import date and time stamp
	uint32_t bound_import_ref::get_timestamp() const noexcept
	{
		return timestamp_;
	}

	//Sets module name
	void bound_import_ref::set_module_name(const std::string& module_name) noexcept
	{
		module_name_ = module_name;
	}

	//Sets timestamp
	void bound_import_ref::set_timestamp(uint32_t timestamp) noexcept
	{
		timestamp_ = timestamp;
	}

	//Default constructor
	bound_import::bound_import() noexcept
		:timestamp_(0)
	{}
	...
	//Returns imported module name
	const std::string& bound_import::get_module_name() const noexcept
	{
		return module_name_;
	}

	//Returns bound import date and time stamp
	uint32_t bound_import::get_timestamp() const noexcept
	{
		return timestamp_;
	}

	//Returns bound references cound
	size_t bound_import::get_module_ref_count() const noexcept
	{
		return refs_.size();
	}

	//Returns module references
	const bound_import::ref_list& bound_import::get_module_ref_list() const noexcept
	{
		return refs_;
	}

	//Adds module reference
	void bound_import::add_module_ref(const bound_import_ref& ref) noexcept
	{
		refs_.push_back(ref);
	}

	//Clears module references list
	void bound_import::clear_module_refs() noexcept
	{
		refs_.clear();
	}

	//Returns module references
	bound_import::ref_list& bound_import::get_module_ref_list() noexcept
	{
		return refs_;
	}

	//Sets module name
	void bound_import::set_module_name(const std::string& module_name) noexcept
	{
		module_name_ = module_name;
	}

	//Sets timestamp
	void bound_import::set_timestamp(uint32_t timestamp) noexcept
	{
		timestamp_ = timestamp;
	}
	...
	const bound_import_module_list get_bound_import_module_list(const pe_base& pe)
	{
		...
		const uint32_t bound_import_data_len =
	...
	const image_directory rebuild_bound_imports(pe_base& pe, const bound_import_module_list& imports, section& imports_section, uint32_t offset_from_section_start, bool save_to_pe_header, bool auto_strip_last_section)
	{
		...
		for (bound_import_module_list::const_iterator it = imports.begin(); it != imports.end(); ++it)
		{
			...
			needed_size_for_strings += gsl::narrow_cast<uint32_t>((*it).get_module_name().length()) + 1 /* nullbyte */;
			...
			{
				needed_size_for_strings += gsl::narrow_cast<uint32_t>((*ref_it).get_module_name().length()) + 1 /* nullbyte */;
		...
		for (bound_import_module_list::const_iterator it = imports.begin(); it != imports.end(); ++it)
		{
			...
			current_pos_for_strings += gsl::narrow_cast<uint32_t>(length);
