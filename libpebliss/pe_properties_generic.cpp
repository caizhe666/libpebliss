...
namespace pe_bliss
{
	...
	{
		return std::unique_ptr<pe_properties>(std::make_unique<pe_properties_generic<PEClassType>>(*this));
	}

	//Fills properly PE structures
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::create_pe(uint32_t section_alignment, uint16_t subsystem) noexcept
	{
	...
	template<typename PEClassType>
	bool pe_properties_generic<PEClassType>::directory_exists(uint32_t id) const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::strip_data_directories(uint32_t min_count, bool strip_iat_directory) noexcept
	{
		...
		for (; i >= 0; i--)
		{
			...
			if (nt_headers_.OptionalHeader.DataDirectory[i].VirtualAddress && (gsl::narrow_cast<uint32_t>(i) != image_directory_entry_iat || !strip_iat_directory))
				break;

			if (i <= gsl::narrow_cast<int>(min_count) - 2)
				break;
		}
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_image_base_32() const noexcept
	{
		return gsl::narrow_cast<uint32_t>(nt_headers_.OptionalHeader.ImageBase);
	}

	//Returns image base for PE32/PE64
	template<typename PEClassType>
	uint64_t pe_properties_generic<PEClassType>::get_image_base_64() const noexcept
	{
		return gsl::narrow_cast<uint64_t>(nt_headers_.OptionalHeader.ImageBase);
	}

	//Sets new image base
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_image_base(uint32_t base) noexcept
	{
	...
	//Sets new image base
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_image_base_64(uint64_t base) noexcept
	{
		nt_headers_.OptionalHeader.ImageBase = gsl::narrow_cast<typename PEClassType::BaseSize>(base);
	}

	//Returns image entry point
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_ep() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_ep(uint32_t new_ep) noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_file_alignment() const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_section_alignment() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_heap_size_commit(uint32_t size) noexcept
	{
		nt_headers_.OptionalHeader.SizeOfHeapCommit = gsl::narrow_cast<typename PEClassType::BaseSize>(size);
	}

	//Sets heap size commit for PE32/PE64
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_heap_size_commit(uint64_t size) noexcept
	{
		nt_headers_.OptionalHeader.SizeOfHeapCommit = gsl::narrow_cast<typename PEClassType::BaseSize>(size);
	}
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_heap_size_reserve(uint32_t size) noexcept
	{
		nt_headers_.OptionalHeader.SizeOfHeapReserve = gsl::narrow_cast<typename PEClassType::BaseSize>(size);
	}

	//Sets heap size reserve for PE32/PE64
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_heap_size_reserve(uint64_t size) noexcept
	{
		nt_headers_.OptionalHeader.SizeOfHeapReserve = gsl::narrow_cast<typename PEClassType::BaseSize>(size);
	}
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_stack_size_commit(uint32_t size) noexcept
	{
		nt_headers_.OptionalHeader.SizeOfStackCommit = gsl::narrow_cast<typename PEClassType::BaseSize>(size);
	}

	//Sets stack size commit for PE32/PE64
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_stack_size_commit(uint64_t size) noexcept
	{
		nt_headers_.OptionalHeader.SizeOfStackCommit = gsl::narrow_cast<typename PEClassType::BaseSize>(size);
	}
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_stack_size_reserve(uint32_t size) noexcept
	{
		nt_headers_.OptionalHeader.SizeOfStackReserve = gsl::narrow_cast<typename PEClassType::BaseSize>(size);
	}

	//Sets stack size reserve for PE32/PE64
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_stack_size_reserve(uint64_t size) noexcept
	{
		nt_headers_.OptionalHeader.SizeOfStackReserve = gsl::narrow_cast<typename PEClassType::BaseSize>(size);
	}
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_heap_size_commit_32() const noexcept
	{
		return gsl::narrow_cast<uint32_t>(nt_headers_.OptionalHeader.SizeOfHeapCommit);
	}

	//Returns heap size commit for PE32/PE64
	template<typename PEClassType>
	uint64_t pe_properties_generic<PEClassType>::get_heap_size_commit_64() const noexcept
	{
		return gsl::narrow_cast<uint64_t>(nt_headers_.OptionalHeader.SizeOfHeapCommit);
	}

	//Returns heap size reserve for PE32
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_heap_size_reserve_32() const noexcept
	{
		return gsl::narrow_cast<uint32_t>(nt_headers_.OptionalHeader.SizeOfHeapReserve);
	}

	//Returns heap size reserve for PE32/PE64
	template<typename PEClassType>
	uint64_t pe_properties_generic<PEClassType>::get_heap_size_reserve_64() const noexcept
	{
		return gsl::narrow_cast<uint64_t>(nt_headers_.OptionalHeader.SizeOfHeapReserve);
	}

	//Returns stack size commit for PE32
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_stack_size_commit_32() const noexcept
	{
		return gsl::narrow_cast<uint32_t>(nt_headers_.OptionalHeader.SizeOfStackCommit);
	}

	//Returns stack size commit for PE32/PE64
	template<typename PEClassType>
	uint64_t pe_properties_generic<PEClassType>::get_stack_size_commit_64() const noexcept
	{
		return gsl::narrow_cast<uint64_t>(nt_headers_.OptionalHeader.SizeOfStackCommit);
	}

	//Returns stack size reserve for PE32
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_stack_size_reserve_32() const noexcept
	{
		return gsl::narrow_cast<uint32_t>(nt_headers_.OptionalHeader.SizeOfStackReserve);
	}

	//Returns stack size reserve for PE32/PE64
	template<typename PEClassType>
	uint64_t pe_properties_generic<PEClassType>::get_stack_size_reserve_64() const noexcept
	{
		return gsl::narrow_cast<uint64_t>(nt_headers_.OptionalHeader.SizeOfStackReserve);
	}

	//Returns virtual size of image
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_size_of_image() const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_number_of_rvas_and_sizes() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_number_of_rvas_and_sizes(uint32_t number) noexcept
	{
	...
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_characteristics() const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_checksum() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_checksum(uint32_t checksum) noexcept
	{
	...
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_dll_characteristics() const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_time_date_stamp() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_time_date_stamp(uint32_t timestamp) noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_dll_characteristics(uint16_t characteristics) noexcept
	{
	...
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_machine() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_machine(uint16_t machine) noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_characteristics(uint16_t ch) noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_size_of_headers() const noexcept
	{
	...
	//Returns subsystem
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_subsystem() const noexcept
	{
	...
	//Sets subsystem
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_subsystem(uint16_t subsystem) noexcept
	{
		nt_headers_.OptionalHeader.Subsystem = subsystem;
	}
	...
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_size_of_optional_header() const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_pe_signature() const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_magic() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_os_version(uint16_t major, uint16_t minor) noexcept
	{
	...
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_minor_os_version() const noexcept
	{
	...
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_major_os_version() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_subsystem_version(uint16_t major, uint16_t minor) noexcept
	{
	...
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_minor_subsystem_version() const noexcept
	{
	...
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_major_subsystem_version() const noexcept
	{
	...
	{
		if (bound_check && gsl::narrow_cast<uint64_t>(va) - nt_headers_.OptionalHeader.ImageBase > pe_utils::max_dword)
			throw pe_exception("Incorrect address conversion", pe_exception::incorrect_address_conversion);

		return gsl::narrow_cast<uint32_t>(va - nt_headers_.OptionalHeader.ImageBase);
	}
	...
	{
		if (!pe_utils::is_sum_safe(rva, gsl::narrow_cast<uint32_t>(nt_headers_.OptionalHeader.ImageBase)))
			throw pe_exception("Incorrect address conversion", pe_exception::incorrect_address_conversion);

		return gsl::narrow_cast<uint32_t>(rva + nt_headers_.OptionalHeader.ImageBase);
	}

	//Relative Virtual Address (RVA) to Virtual Address (VA) convertions for PE32/PE64
	template<typename PEClassType>
	uint64_t pe_properties_generic<PEClassType>::rva_to_va_64(uint32_t rva) const noexcept
	{
		return gsl::narrow_cast<uint64_t>(rva) + nt_headers_.OptionalHeader.ImageBase;
	}

	//Returns number of sections
	template<typename PEClassType>
	uint16_t pe_properties_generic<PEClassType>::get_number_of_sections() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_number_of_sections(uint16_t number) noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_size_of_image(uint32_t size) noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_size_of_headers(uint32_t size) noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_size_of_optional_header(uint16_t size) noexcept
	{
	...
	template<typename PEClassType>
	char* pe_properties_generic<PEClassType>::get_nt_headers_ptr() noexcept
	{
	...
	//Returns nt headers data pointer
	template<typename PEClassType>
	const char* pe_properties_generic<PEClassType>::get_nt_headers_ptr() const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_sizeof_nt_header() const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_sizeof_opt_headers() const noexcept
	{
	...
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_file_alignment_unchecked(uint32_t alignment) noexcept
	{
	...
	//Sets base of code
	template<typename PEClassType>
	void pe_properties_generic<PEClassType>::set_base_of_code(uint32_t base) noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_base_of_code() const noexcept
	{
	...
	template<typename PEClassType>
	uint32_t pe_properties_generic<PEClassType>::get_needed_magic() const noexcept
	{
	...
	template<typename PEClassType>
	pe_type pe_properties_generic<PEClassType>::get_pe_type() const noexcept
	{
