...
namespace pe_bliss
{
	...
	pe_base::pe_base(std::istream& file, const pe_properties& props, bool read_debug_raw_data)
	{
		...
		const std::streamoff old_offset = file.tellg();
	...
	//Returns dos header
	const image_dos_header& pe_base::get_dos_header() const noexcept
	{
		return dos_header_;
	}

	//Returns dos header
	image_dos_header& pe_base::get_dos_header() noexcept
	{
		return dos_header_;
	}

	//Returns PE headers start position (e_lfanew)
	int32_t pe_base::get_pe_header_start() const noexcept
	{
		return dos_header_.e_lfanew;
	}

	//Strips MSVC stub overlay
	void pe_base::strip_stub_overlay() noexcept
	{
		rich_overlay_.clear();
	}
	...
	const std::string& pe_base::get_stub_overlay() const noexcept
	{
	...
	{
		const uint16_t new_number = gsl::narrow_cast<uint16_t>(sections_.size());
